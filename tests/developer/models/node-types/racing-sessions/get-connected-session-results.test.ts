...
describe('Collect connected SESSION RESULTS for the RACING SESSION detail page', () => {
    ...
    test('when there are SESSION RESULTS connected', async () => {
        vi.spyOn(RacingSessionDataFacade, 'getConnectedSessionResultNodes').mockResolvedValue([
            {id: 12222228, name: "dummy 2", partner_node: FakeSessionResult.data} as unknown as RacingSessionHasSessionResultRelationship,
            {id: 13333338, name: "dummy 3", partner_node: FakeSessionResult.data} as unknown as RacingSessionHasSessionResultRelationship,
        ])
