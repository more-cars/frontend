...
describe('Fetching connected CAR MODEL VARIANTS from data source', () => {
    test('when there are no CAR MODEL VARIANTS connected', async () => {
        ...
        const {getConnectedCarModelVariants} = await import("../../../../../src/data/node-types/motor-shows/getConnectedCarModelVariants")
    ...
    test('when the MOTOR SHOW does not exist', async () => {
        vi.doMock("../../../../../src/data/node-types/motor-shows/getMotorShowById", () => ({
            getMotorShowById: vi.fn(() => null)
        }))
