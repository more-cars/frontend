...
describe('Collect connected <%= h.changeCase.upper(h.inflection.pluralize(partnerNodeType)) %> for the <%= h.changeCase.upper(nodeType) %> detail page', () => {
    ...
    test('when there are <%= h.changeCase.upper(h.inflection.pluralize(partnerNodeType)) %> connected', async () => {
        vi.spyOn(<%= h.changeCase.pascal(nodeType) %>DataFacade, 'getConnected<%= h.changeCase.pascal(partnerNodeType) %>Nodes').mockResolvedValue([
            {id: 12222228, name: "dummy 2", partner_node: Fake<%= h.changeCase.pascal(partnerNodeType) %>.data} as unknown as <%= h.changeCase.pascal(nodeType) %><%= h.changeCase.pascal(relationshipName) %>Relationship,
            {id: 13333338, name: "dummy 3", partner_node: Fake<%= h.changeCase.pascal(partnerNodeType) %>.data} as unknown as <%= h.changeCase.pascal(nodeType) %><%= h.changeCase.pascal(relationshipName) %>Relationship,
        ])
